...
parser = argparse.ArgumentParser(description='Receive measurement values from a DL24/DL24P electonic DC load using BLE')
...
class NotifyRequester(GATTRequester):
    ...
    def on_notification(self, handle, data):
        if(len(data) == 23):
            ...
            power = voltage/10*current/1000
            if voltage > 0 and current > 0:
                resistance = (voltage/10)/(current/1000)
            else:
                resistance = 0
            energy = int.from_bytes(self.fulldata[0x10:0x14], 'big', signed=False)*10
            hours = int.from_bytes(self.fulldata[0x20:0x21], 'big', signed=False)
            minutes = int(self.fulldata[0x22])
            seconds = int(self.fulldata[0x23])
            temperature = int(self.fulldata[0x1f])
            

            if args.mode == 'Text':
            
                data = [
                    ("    Voltage: {:8.2f}V".format(voltage/10)),
                    ("    Current: {:8.2f}A".format(current/1000)),
                    ("      Power: {:8.2f}W".format(power)),
                    (" Resistance: {:8.2f}Ω".format(resistance)),
                    "",
                    ("   Capacity: {:5}   mAh".format(capacity*10)),
